...
import com.muxiaohong.kotlinlib.event.Message
import com.muxiaohong.kotlinlib.event.SingleLiveEvent
import com.muxiaohong.kotlinlib.network.ExceptionHandle
...
open class BaseViewModel : AndroidViewModel(Utils.getApp()), LifecycleObserver {
     ...
     */
    fun launchGo(
        pre: suspend CoroutineScope.() -> Unit = {},
    ...
    private suspend fun <T> executeResponse(
        ...
        coroutineScope {
            LogUtils.d(response.msg())
            if (response.isSuccess()) success(response.data())
            else throw Throwable(response.msg())
        }
    }
